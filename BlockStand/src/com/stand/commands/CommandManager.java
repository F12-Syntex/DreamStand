...
import com.stand.config.Cooldown;
import com.stand.config.Messages;
import com.stand.config.Permissions;
import com.stand.cooldown.CooldownUser;
import com.stand.main.Stand;
import com.stand.tags.TagFactory;
import com.stand.utils.MessageUtils;
import com.stand.utils.StringMinipulation;

public class CommandManager implements CommandExecutor {

    private ArrayList<SubCommand> commands = new ArrayList<SubCommand>();
    private Messages messages = Stand.getInstance().configManager.messages;
    private Permissions permissions = Stand.getInstance().configManager.permissions;
    private Cooldown cooldowns = Stand.getInstance().configManager.cooldown;
    
    private Stand plugin;

    //Sub Commands
    public String main = "stand";
    
    public void setup(Stand plugin) {
    	this.setPlugin(plugin);
    	plugin.getCommand(main).setExecutor(this);
        commands.add(new Blocks());
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase(main)) {
            ...
            if (args.length == 0) {
            	...
            	CooldownUser user = Stand.getInstance().cooldownManager.getUser(player.getUniqueId());
	...
	public Stand getPlugin() {
		return plugin;
	}


	public void setPlugin(Stand plugin) {
		this.plugin = plugin;
	}
