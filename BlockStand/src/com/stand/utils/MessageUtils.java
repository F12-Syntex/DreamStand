...
public class MessageUtils {
	...
	public static void inform(Player player, String s) {
		player.sendMessage(Stand.getInstance().configManager.messages.prefix + MessageUtils.translateAlternateColorCodes(s));
	}
	...
	public static void sendHelp(Player player) {
		 Stand.getInstance().CommandManager.getCommands().forEach(i -> {
			 MessageUtils.sendRawMessage(player,Stand.getInstance().configManager.messages.prefix + " " + "&6• &c" + i.name() + "&7 : &b" + i.info());
		 });
	}
}
